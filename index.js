//I'll start by defining the two classes to be used
class Bookshelf {
    //For future projects, I'd like to have bookshelves with limited space so that my website could display a stack of shelves of a given capacity, instead of a single horizontal shelf - so I've given the constructor function these parameters for later use, even though I'm overwriting all of them for this project.
    constructor(capacity, upFromFloor, contents) {
        this.capacity = 1000000;
        this.upFromFloor = 0;
        this.contents = [];
    }
    addBook(obj) {
        //I want my addBook function to store a new object in the `contents` array, print a confirmation message for the user, and display the new bookshelf on the browser.
        this.contents.push(obj);
        console.log(`A book has been added to the shelf!  ${obj.title}`);
        //this.render();
    }
    removeBook(pos) {
        console.log(`A book has been removed from the shelf!  ${this.contents[pos-1].title}`);
        //I didn't see a way to delete interior items using the slice method, so I looked up the splice method. I also had to log the message before manipulating the array so that I could refer to the object about to be deleted.
        this.contents.splice(pos-1, 1);
        //this.render();
    }
    render() {
        //Originally I called this function at the end of my `addBook` and `removeBook` methods, but I couldn't figure out how to get the function to only append one child once and then replace the child every time after. So currently, I have to render the bookshelf separately after changing the books. :-/
        let shelfEle = document.createElement("section");
        shelfEle.setAttribute("id", "innards")
        for(let i = 0; i < this.contents.length; i++) {
            ...
            //fill child elements
            titleEle.innerHTML = `${this.contents[i].title}`;
            //I wanted to make the book display look a little more authentic by displaying the author's first name on one line and last name on another - I researched the `split` method and found it quite up to the task, except for some author's whose names have abbreviations that get expanded in the data but don't need to be displayed that way, which would be a edge-case scenario to improve upon later in life.
            authorEle.innerHTML = `${this.contents[i].author[0].split(', ')[1]}`;
            authorEle.innerHTML += `<p>${this.contents[i].author[0].split(', ')[0]}`;
            //append parent element
            shelfEle.appendChild(bookEle);
        }
        document.body.appendChild(shelfEle);
    }
}
...
class Book {
    ...
    getColor() {//I want to use this function to generate colors for my books' display properties, though I'm starting off easy and limiting the possible colors to only some shades of gray so that a single color of text will always be more readable on all the books.
        let randCode = Math.floor(Math.random() * 65 + 1);
        let hexCode = `${randCode}${randCode}${randCode}`;
        return hexCode;
    }
}

//And unless Keiran has that video posted, I'll just copy and paste `bookData` into here because I don't know how to import/export
const bookData = [
    {
...
//Now I need to add my bookData to my bookshelf...
const grayShelf = new Bookshelf;
for(let i = 0; i < bookData.length; i++) {//for rendering practice, I am going to change bookData.length to 3, then hopefully I'll remember to change it back once everything is rendering well before I turn it in.
    const newBook = new Book(bookData[i].author, bookData[i].language, bookData[i].subject, bookData[i].title);
    grayShelf.addBook(newBook)
}

//...render my shelf...
grayShelf.render();

//...and colorize the books!
let bookList = document.querySelectorAll('.Book');
      console.log(bookList[0]);
for(let i = 0; i < bookList.length; i++) {
  bookList[i].style.color = `#aaa`;
  bookList[i].style.background = `#${grayShelf.contents[i].getColor()}`;
  if (grayShelf.contents[i].title.length > 180) {
    bookList[i].style.width = '650px';
  }
}
